		...
		$rspta=$libro->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
				 ...
				 ' <button class="btn btn-primary" onclick="activar('.$reg->idlibro.')"><i class="fa fa-check"></i></button>').'<button class="btn btn-danger" onclick="eliminarFila('.$reg->idlibro.')"><i class="fa fa-trash"></i></button>' ,
 				"1"=>$reg->titulo,
...
case "SelectEditorial":
 ...
 case "SelectMateria":
	...
	case 'eliminar':
		$rspta=$libro->eliminar($idlibro);

		$error_message =$rspta;
		$log_file = "./my-errors.log"; 
		error_log($error_message, 3, $log_file); 

		if( $rspta == "Cannot delete or update a parent row: a foreign key constraint fails (`sistemabiblioteca`.`prestamo`, CONSTRAINT `fk_prestamo_libro` FOREIGN KEY (`idlibro`) REFERENCES `libro` (`idlibro`))")
			echo $rspa; 
		else
			echo $rspta ? "Libro Eliminado" : "Libro no se puede eliminar";
